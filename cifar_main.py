...
from cifar_utils import load_cifar10, load_model
...
def get_args():
    ...
    parser.add_argument('--model-name', default='resnet4b', type=str)
...
def main():
    ...
    if args.cw:
        post_conf = cw_loss(opt_out, label).item()
    else:
        post_conf = reachability_loss(opt_out, label).item()
