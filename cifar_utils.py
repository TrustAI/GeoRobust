...
def load_cifar10(dir_, batch_size):
...
def load_model(model_dir, model_name, device):
    if model_name[-4:] == 'onnx':
        torch_model = convert(os.path.join(model_dir,model_name))
        return torch_model.to(device)
    ckpt = torch.load(os.path.join(model_dir,model_name+'.pth'))
    if model_name == 'resnet4b':
        model = resnet4b()
    elif model_name == 'resnet2b':
        model = resnet2b()
